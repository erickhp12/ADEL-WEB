...
export default class extends React.Component {
  ...
  addAppointment = () => {
    ...
    let duracion = this.state.servicios_duracion.find(
      o => o.id === this.state.selected_service.value
    ).duration;
    let costo = parseFloat(
      this.state.servicios_duracion.find(
        o => o.id === this.state.selected_service.value
      ).price
    );
    fecha = moment(fecha)
  ...
  selectService = async selected_service => {
    ...
    let duration = this.state.servicios_duracion.find(o => o.id === service)
      .duration;
    ...
    try {
      const params = {
        branch_ofice: localStorage.getItem("sucursal"),
        services: service
      };
  ...
  selectAssociate = selected_associate => {
    ...
    let associate_index = this.state.associates.findIndex(
      o => o.value === associate
    );
              ...
              {this.state.data.appointments_associates.length > 0 ? (
                <div>
                  <table className="table is-fullwidth is-striped is-hoverable is-bordered">
                    ...
                    <tbody>
                      {this.state.data.appointments_associates.map(
                        (obj, index) => (
                          <tr key={index}>
                            ...
                            <td>
                              <p className="buttons is-centered">
                                <a
                                  onClick={e =>
                                    this.deleteAppointMentAssociate(index)
                                  }
                                  className="button is-small is-danger tooltip"
                                  data-tooltip="Borrar"
                                >
